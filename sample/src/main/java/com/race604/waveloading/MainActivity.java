...
public class MainActivity extends AppCompatActivity {
	...
	private WaveDrawable mColorWave;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mRadioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				setIndeterminateMode(mWaveDrawable, indeterminate);
			}
		});
		setIndeterminateMode(mWaveDrawable, mRadioGroup.getCheckedRadioButtonId() == R.id.rb_yes);
...
//        chromeWave.setIndeterminateAnimator(animator);
		...
		mColorWave = new WaveDrawable(new ColorDrawable(R.color.colorAccent));
		view.setBackground(mColorWave);
		setIndeterminateMode(mColorWave, false);
//		mColorWave.setLevel(5000);
	}

	private void setIndeterminateMode(WaveDrawable wave, boolean indeterminate) {
		wave.setIndeterminate(indeterminate);
		mLevelSeekBar.setEnabled(!indeterminate);

		if (!indeterminate) {
			wave.setLevel(mLevelSeekBar.getProgress());
		}
		wave.setWaveAmplitude(mAmplitudeSeekBar.getProgress());
		wave.setWaveLength(mLengthSeekBar.getProgress());
		wave.setWaveSpeed(mSpeedSeekBar.getProgress());
	}
